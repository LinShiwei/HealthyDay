    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        healthManager.readCurrentStepCount{ [unowned self] (count,error) in
            if count != nil && error == nil{
                DispatchQueue.main.async {
                    self.footStepCoundView.footStepCountLabel.text = "\(count!) Steps"
                }
            }else{
                DispatchQueue.main.async {
                    self.footStepCoundView.footStepCountLabel.text = "0 Step"
                }
            }
        }
        healthManager.readDistanceWlakingRunning{ [unowned self] (distance,error) in
            if distance != nil && error == nil{
                DispatchQueue.main.async {
                    self.distanceView.distanceLabel.text = "\(distance!) Meters"
                }
            }else{
                DispatchQueue.main.async {
                    self.distanceView.distanceLabel.text = "0 Meter"
                }
            }
        }
        #endif
